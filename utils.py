...
def daily_aggregates_per_admin(df, rainfall_thresholds, save_to_file=None, save_fig_to_png=None):
    """
...
def plot_rainfall_map_per_day(rainfall_da, settings, shapefile_fldr, destination_fldr, location_name, timestamp):
    """
    ...
    cmap = (mpl.colors.ListedColormap(['#FFFFFF', '#ffffcc', '#a1dab4', '#41b6c4', '#2c7fb8', '#253494'])).with_extremes(over='#1C286E') # YlGnBu 1+5 scale
    cbar_kwargs = {
    ...
    for nmbr_days in tqdm(rainfall_da.hours_ahead.values):
        ...
        for admin_lvl in admin_levels:
            if settings['geoCoordinates'][admin_lvl]: 
                ...
                try:
                    adm.boundary.plot(ax = ax, color="darkgrey", linewidth = 0.75, linestyle='dashed')
                # Otherwise it is a point/ a city  
                except: 
                    ax.scatter(data=adm, x='x_mean', y='y_mean', label='group_village_head_name')
