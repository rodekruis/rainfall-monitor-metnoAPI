...
def collect_rainfall_data(settings_file, remove_temp, store_in_cloud):
    ...
    cloud_all_output = os.path.join(cloud_results_dir, f"{now_stamp}", settings['AzureCloudStorage']['raw_output'])
    ...
    local_all_output = os.path.join(local_results, f"{now_stamp}", settings['localStorage']['raw_output'])
    local_png_dir = os.path.join(local_results, f"{now_stamp}", settings['localStorage']['figures_dir'])
    ...
    # rainfall_gdf = gpd.read_file(os.path.join(local_all_output,file_geotable))
    ...
    rainfall_array = gdf_to_rasterfile(
        rainfall_gdf, 
        save_to_file=os.path.join(local_all_output,file_raster)
        )
    ...
    for admin_lvl in admin_levels:
        ...
        if settings['geoCoordinates'][admin_lvl]:  
            ...
            for band_idx, timepoint in tqdm(enumerate(rainfall_array.time_of_prediction)):
                ...
                by_admin = zonal_statistics(rasterfile=os.path.join(local_all_output,file_raster),
            ...
            rainfall_by_admin.to_csv(
                os.path.join(local_all_output,filename_zonal_stats_admin), 
                index=False)
            ...
            rainfall_by_admin_by_day, by_admin_by_day = \
                ...
                save_to_file=os.path.join(local_all_output,file_zonal_daily_admin), 
                save_fig_to_png=os.path.join(local_png_dir,file_bar_plot_admin),
            ...
            check_threshold(rainfall_by_admin_by_day, 
                            'ONE-DAY', 
                            save_to_file=os.path.join(local_all_output,file_trigger_admin))
